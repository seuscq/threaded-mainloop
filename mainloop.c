...
#define DEBUG
  ...
  #define pr_info(format, ...) \
	  fprintf(stderr, "threaded-mainloop: %s():" format "\n", __func__, ##__VA_ARGS__)
#else
  #define pr_info
#endif
...
void mainloop_quit(void)
{
	...
	tdata = find_thread_data_by_tid(tid);
	if (!tdata) {
		// warn user no thread_data found
		pr_info("no tdata\n");
		return;
	}
...
int mainloop_run(void)
{
	...
	for (i = 0; i < MAX_MAINLOOP_ENTRIES; i++) {
		struct mainloop_data *data = tdata->mainloop_list[i];
	...
	// TODO: remove tdata from list and free it here;
	pthread_rwlock_wrlock(&threads_lock);
